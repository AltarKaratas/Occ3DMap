...
OccupancyGrid::OccupancyGrid(const double voxel_size)
...
void OccupancyGrid::integrate(const std::vector<Eigen::Vector3d> &cloud, const Eigen::Matrix4d &pose)
{
    ...
    const Eigen::Vector3d sensor_origin = pose.block<3, 1>(0, 3);
    ...
    for (const auto &pt : cloud)
    {
        ...
        // Point to world
        const Eigen::Vector4d homo_pt(pt.x(), pt.y(), pt.z(), 1.0);
        // this is a matrix mul
        const Eigen::Vector4d pt_in_map = pose * homo_pt;

        const VoxelKey end_voxel = {
        ...
        std::vector<OccupancyGrid::VoxelKey> bres_line = bresenham3D(start_voxel, end_voxel);
        ...
        for (const auto &voxel : bres_line)
        {
            ...
            {
                double &cell = m_grid[voxel];
                cell = std::clamp(cell + m_loggodds_free,
                                           m_loggodds_min, m_loggodds_max);
                visited_voxels.insert(voxel);
            }
        }
        // update the occupied cells' value
        double& ending_cell = m_grid[end_voxel];
        ending_cell = std::clamp(ending_cell + m_loggodds_occ,m_loggodds_min, m_loggodds_max);
    }
}
...
std::vector<Eigen::Vector3d> OccupancyGrid::DownsamplePCD(const std::vector<Eigen::Vector3d> &cloud,const double voxel_size) const
{
    std::unordered_set<VoxelKey, VoxelKeyHasher> seen_voxels;
    std::vector<Eigen::Vector3d> downsampled;

    for (const auto& pt : cloud) {
        const int ix = static_cast<int>(std::floor(pt.x() / voxel_size));
        const int iy = static_cast<int>(std::floor(pt.y() / voxel_size));
        const int iz = static_cast<int>(std::floor(pt.z() / voxel_size));
