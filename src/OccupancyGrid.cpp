...
void OccupancyGrid::integrate(const std::vector<Eigen::Vector3d>& cloud, const Eigen::Matrix4d& pose){
    ...
    for (const auto& pt : cloud) {
        ...
        m_grid[end_voxel] = std::clamp(m_grid[end_voxel] + m_loggodds_occ,
                                       m_loggodds_min, m_loggodds_max);
    
        auto t5 = std::chrono::high_resolution_clock::now();
    
        std::cout << "Transform: " << std::chrono::duration<double>(t1 - t0).count()
                  << " | EndVoxel: " << std::chrono::duration<double>(t2 - t1).count()
                  << " | Bresenham: " << std::chrono::duration<double>(bres_end - bres_start).count()
                  << " | Free update: " << std::chrono::duration<double>(t4 - t3).count()
                  << " | Occupied update: " << std::chrono::duration<double>(t5 - t4).count()
                  << std::endl;   
    }
...
std::vector<OccupancyGrid::VoxelKey> OccupancyGrid::bresenham3D(const VoxelKey& start, const VoxelKey& end) const{
    ...
    if (dx >= dy && dx >= dz) {
        ...
        for (int i = 0; i <= dx; ++i) {
            ...
            if (err_y > 0) { y0 += sy; err_y -= dx2; }
            if (err_z > 0) { z0 += sz; err_z -= dx2; }
            err_y += dy2;
            err_z += dz2;
            x0 += sx;
        }
        ...
        for (int i = 0; i <= dy; ++i) {
            line.push_back({x0, y0, z0});
            if (err_x > 0) { x0 += sx; err_x -= dy2; }
            if (err_z > 0) { z0 += sz; err_z -= dy2; }
            err_x += dx2;
            err_z += dz2;
            y0 += sy;
        }
        ...
        for (int i = 0; i <= dz; ++i) {
            line.push_back({x0, y0, z0});
            if (err_x > 0) { x0 += sx; err_x -= dz2; }
            if (err_y > 0) { y0 += sy; err_y -= dz2; }
            err_x += dx2;
            err_y += dy2;
            z0 += sz;
        }
